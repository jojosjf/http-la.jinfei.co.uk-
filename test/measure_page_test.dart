...
Future<LoadedImage> testImage(WidgetTester tester) async {
  final image = await tester.runAsync(() => createTestImage(width: 100, height: 140));
...
void main() {
  ...
  testWidgets('默认线位置为 50/50，PSA 10', (tester) async {
    ...
    for (final n in ['outerLeft', 'innerLeft', 'innerRight', 'outerRight',
  ...
  testWidgets('拖动中结果实时变化', (tester) async {
    ...
    String lr() => (tester.widgetList<Text>(find.textContaining('/'))
            .map((t) => t.data!)
            .where((t) => t != '50/50'))
        .single;
    final first = lr();
  ...
  testWidgets('点手柄选中，点空白取消', (tester) async {
    final img = await testImage(tester);
    await tester.pumpWidget(app(MeasureView(image: img)));
    await tester.tap(handle('outerTop'));
    await tester.pump();
    await tester.tapAt(tester.getCenter(find.byType(MeasureView)) - const Offset(0, 150));
    await tester.pump();
  });
}
